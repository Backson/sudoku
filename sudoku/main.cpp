...
void test_depth_search_solver(const sudoku::Puzzle puzzle) {
	typedef chrono::steady_clock clock;
	auto start = clock::now();
	sudoku::DepthSearchSolver solver;
	auto solutions = solver.solve(puzzle);
	auto end = clock::now();
	auto duration = end - start;
	auto nanoseconds = chrono::duration_cast<chrono::nanoseconds>(duration).count();
...
int main() {
	...
	*/
	...
	cout << "Testing the DepthSearchSolver:" << endl;
	test_depth_search_solver(puzzle);
	cout << endl;
